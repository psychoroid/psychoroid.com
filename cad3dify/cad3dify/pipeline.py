...
def generate_step_from_text(
    text_prompt: str,
    output_filepath: str,
    num_refinements: int = 3,
    model_type: MODEL_TYPE = "gpt",
):
    """Generate a STEP file from a text description

    Args:
        text_prompt (str): Text description of the desired 3D model
        output_filepath (str): Path to the output STEP file
        num_refinements (int, optional): Number of refinement iterations. Defaults to 3.
        model_type (MODEL_TYPE, optional): Type of LLM to use. Defaults to "gpt".
    """
    only_execute = (model_type == "llama")
    chain = CadCodeGeneratorChain(model_type=model_type)

    # Generate initial code from text prompt
    result = chain.invoke({"text": text_prompt})["result"]
    code = result.format(output_filename=output_filepath)
    logger.info("1st code generation complete. Running code...")
    logger.debug("Generated 1st code:")
    logger.debug(code)
    ...
    for i in range(num_refinements):
        with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as f:
            ...
            rendered_image = ImageData.load_from_file(f.name)
            result = refiner_chain.invoke(
                {"code": code, "original_input": text_prompt, "rendered_result": rendered_image}
            )["result"]
            if result is None:
                logger.error("Refinement failed. Skipping to the next step.")
                continue
