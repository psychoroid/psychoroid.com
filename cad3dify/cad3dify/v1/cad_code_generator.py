...
_text_prompt_template = """You are a CAD expert using CadQuery to create 3D models. Your task is to generate CadQuery Python code that creates a 3D model based on the following text description:

{text}

Please follow these guidelines:
1. Use CadQuery's Python API to create the model
2. Follow best practices for parametric modeling
3. Add comments to explain key steps
4. Use appropriate workplanes and operations
5. Ensure the model is manufacturable
6. Include all necessary imports
7. The final variable should be named 'result'

Here are some example CadQuery code snippets for reference:

{examples}

Please generate the code following the design steps:

{design_steps}

Generate only the Python code without any additional text or markdown formatting."""
...
class CadCodeGeneratorChain(SequentialChain):
    ...
    def __init__(self, model_type: MODEL_TYPE = "gpt") -> None:
        ...
        chat_model = ChatModelParameters(model_type=model_type).get_model()

        # Text-based prompt template
        text_prompt = ChatPromptTemplate(
            messages=[
                HumanMessagePromptTemplate(
                    prompt=PromptTemplate(
                        template=_text_prompt_template,
                        input_variables=["text"],
                        partial_variables={
                            "examples": "\n\n".join(
                                f"Example {i+1} ({desc}):\n{code}"
                                for i, (desc, code) in enumerate(_cad_query_examples)
                            ),
        ...
        llm_chain = LLMChain(llm=chat_model, prompt=text_prompt)
    ...
    def prep_inputs(self, inputs: Union[dict[str, Any], Any]) -> dict[str, str]:
        ...
        elif isinstance(inputs, ImageData):
            # Handle direct ImageData input
            return super().prep_inputs({"image": inputs})
